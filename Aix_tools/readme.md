...
- 心情日历增强：新增年度情绪热力图（ECharts calendar 视图），支持年份切换，按情绪强度可视化全年日记情绪分布。

## 最新版本 v0.65.0
- 数据总览增强：新增近 7 天模块健康得分趋势折线图，展示事项/专注/习惯/日记/目标五维得分变化。
- 专注排行榜增强：新增各模式日 TOP 5 排名（倒计时/正计时/番茄钟分别展示最高专注日）。
- 习惯热力图增强：新增年度视图切换，支持在近 6 个月与全年打卡热力图之间切换。
- 目标时间线增强：点击里程碑可打开详情弹窗，展示里程碑状态、所属目标信息和整体进度。

## 最新版本 v0.64.0
- 习惯统计增强：新增习惯月度打卡热力图（本月每一天的打卡次数 ECharts calendar heatmap）。
- 日记统计增强：新增近 30 天情绪强度趋势折线图，按日记情绪强度平均值绘制。
- 事项统计增强：新增事项类型分布饼图（环形图），替代原来的纯数字卡片展示。
- 成长月报增强：新增右上角 Markdown 导出按钮，支持导出本月成长得分、核心指标、目标进度和本月亮点。

## 最新版本 v0.63.0
- 17 个分析页面空状态补齐：为全部 13 个缺少空状态的分析页面添加了 Empty 组件，数据不足时显示友好占位提示。

## 最新版本 v0.62.0
- 成就中心增强：增加近期解锁动态（近5条解锁时间排序展示），解锁时间戳持久化到 cacheKv。
- 专注统计增强：新增24小时专注热力图（近7天×24小时时段分布），按天×小时矩阵展示专注分钟数。
- 数据总览增强：模块健康明细已补齐，5个模块得分进度条可视化。

## 最新版本 v0.61.0
- 年度回顾增强：新增与上年同比对比标签(事项完成/专注时长/习惯打卡/日记篇数)，支持右上角 Markdown 年度复盘导出。
- 复习统计增强：新增掌握率趋势折线图(近30天分6段)、节点效率进度条(D1/D2/D4/D7/D15/D30各节点掌握率可视化)，支持 Markdown 复习报告导出。
- 空状态优化：年度回顾和复习统计页面新增空状态占位提示和导出按钮联动。

## 最新版本 v0.60.0
- 年度回顾：新增全年多维数据聚合复盘页面，支持年份切换、月度趋势四轴图、事项完成率走势、全年核心指标统计。
- 复习统计：新增记忆曲线复习效果分析页面，包含近14天复习趋势、各记忆曲线节点掌握率分布、连续复习天数和同比对比。
- 成长仪表盘深度分析扩展：从 14 个入口扩展到 16 个，新增年度回顾和复习统计快捷入口。
- 全部 17 个分析页面交叉导航补齐。

## 最新版本 v0.59.0
- 数据总览：新增全模块核心指标单页概览，聚合事项、专注、习惯、日记、目标、备忘录六大模块数据，含系统健康度百分制评分、7天趋势双轴图和模块健康雷达。
- 成长仪表盘深度分析扩展：从 13 个入口扩展到 14 个，新增数据总览快捷入口。
