...
- **跨模块关联洞察引擎**: 成长仪表盘新增"黑科技引擎"模块，自动统计分析习惯、专注、情绪、目标之间的隐藏关联，生成如"运动日专注高28%"、"冥想让心情更好"等自然语言洞察
- **智能周期成长报告**: 新增 `/home/growth/report` 页面，支持周/月/季/年四个周期维度，包含每日专注趋势图、情绪分布饼图、习惯完成率水平条等可视化图表，支持导出 JSON 和 HTML
- **进阶成就与等级系统**: 成就从 10 项扩充至 22 项（新增双月王者、百日传奇、完美一周等），新增加 10 级成长等级系统（Lv.1-Lv.10，从"初识成长"到"Aix至尊"），每日 XP 经验值追踪（习惯/专注/日记/事项/目标/备忘均可获得 XP）
...
| **成长警报系统** | 首页聚合目标风险、习惯中断、复习压力信号，新增事项过载雷达，专注能量曲线和日记情绪趋势补齐反馈闭环 |
...
产物: `desktop/dist-installer/AixSystems-0.24.0-Setup.exe` (NSIS 安装包,约 80MB)

目录便携版: `npm run dist:portable` → `desktop/dist-installer/win-unpacked/`

单文件便携版: `npm run dist:portable-exe` → `AixSystems-0.24.0-portable.exe`
