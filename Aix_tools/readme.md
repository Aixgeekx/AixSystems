...
| **成长风险控制** | 目标风险预警 + 习惯恢复计划 + 未来 30 天复习压力热力图 |
...
产物: `desktop/dist-installer/AixSystems-0.23.3-Setup.exe` (NSIS 安装包,约 80MB)

目录便携版: `npm run dist:portable` → `desktop/dist-installer/win-unpacked/`

单文件便携版: `npm run dist:portable-exe` → `AixSystems-0.23.3-portable.exe`
