...
| **复习中心** | 聚合今日待复习、未来 7 天分布、已过期复习、待反馈提醒、完成记录、掌握率和巩固回流 |
...
产物: `desktop/dist-installer/AixSystems-0.22.7-Setup.exe` (NSIS 安装包,约 80MB)

目录便携版: `npm run dist:portable` → `desktop/dist-installer/win-unpacked/`

单文件便携版: `npm run dist:portable-exe` → `AixSystems-0.22.7-portable.exe`
