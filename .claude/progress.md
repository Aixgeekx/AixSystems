...
- [x] 提交并推送 `origin main`
- [x] 创建并推送 tag：`v0.35.0`
- [x] 创建 GitHub Release 并上传 `AixSystems-0.35.0-Setup.exe` 与 blockmap

## 验证与发布结果
- Vitest：6 个测试文件通过，39/39 用例通过。
- 前端构建：`npm --prefix code run build` 通过；仅保留既有字体运行时解析和大 chunk 警告。
- 桌面构建：`npm --prefix desktop run dist` 通过，生成 `AixSystems-0.35.0-Setup.exe` 与 blockmap。
- Git：`main` 已推送到 `origin`，tag `v0.35.0` 已推送。
- Release：`v0.35.0` 已创建/补齐，并上传安装包与 blockmap。
