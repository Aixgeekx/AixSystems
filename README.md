...
AixSystems 是一个**完整可运行的时间管理系统原型框架**,覆盖 23 个业务模块、17 种事项类型、15 款内置主题风格。所有数据本地 IndexedDB 存储,零服务器依赖。
...
|---|---|
| **视图** | 我的一天 / 一周 / 一月 / 一年 |
| **首页工作台** | 打开即见全局总览 / 主题联动主视觉 / 快捷入口 / 本地备份状态 |
| **存储控件** | 首页显示应用占用空间 / 磁盘已用 / 磁盘剩余 |
| **本地搜索体验** | 最近搜索词本地保存 / 点击标签快速复用 |
| **界面质感** | 15 款主题 / 自动切换 / 分组导航更明确 |
...
| **备忘录** | 富文本 + 文件夹 + 置顶 + 回收站 |
| **主题** | 赛博朋克 / 矩阵终端 / 森林氧气 / 余烬夜幕 / 极简 / 渐变 / 复古等 15 款主题风格 |
...
测试     Vitest + @testing-library/react (36 用例)
```
...
产物:`desktop/dist-installer/AixSystems-0.19.2-Setup.exe`(约 80MB NSIS 向导包)
